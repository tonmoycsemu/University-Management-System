<?php include('header.php'); ?>
<?php include('dbconnect.php'); ?>
  <body>
    <div class="container-fluid">
      <div class="row">
        <div class="col-md-3">
            <?php include ('leftmenu.php'); ?>
        </div> <!-- end left -->
        <div class="col-md-9">
          <br>
          <div class="row">
            <div class="col-md-7">
                <h2>Notice Board Registration</h2>
            </div>
            <div class="col-md-5 text-right">
                <a href="noticeBoard.php" class="btn btn-success ">Go Main Page</a>
                <a href="noticeBoard.php?entry" class="btn btn-primary ">Data Entry</a>
            </div>
          </div>

          <br/><br/>
          
          <?php 

                  $msg=NULL;
                 
                  if (isset($_GET['deleteid'])) {
                    $id=$_GET['deleteid'];
                    $deleteq="DELETE FROM noticeboard WHERE id=$id";
                      if(mysqli_query($con,$deleteq)){
                       
                        $msg="<span style='color:green'>Deleted Successfully</span>";
                      }
                      else{
                        $msg="<span style='color:red'>Delete Failed</span>";
                      }
                  }
          ?>

           <?php 

                  

                  if (isset($_POST['update'])) { 
                      $title = addslashes($_POST['title']); 
                      $details = addslashes($_POST['details']);
                      $date = $_POST['date'];
                      $updateid = $_POST['updateid'];

                      $uq="UPDATE noticeboard set title='$title',details='$details',dates='$date' WHERE id=$updateid";
                      $res=mysqli_query($con,$uq);
                      if ($res) {
                        $msg='<div class="alert alert-success" role="alert">Post added successfully!</div>';
                      }
                      else{
                        $msg='<div class="alert alert-warning" role="alert">Failed!</div>';
                      }
                }
                 
                  
             ?>
          
          <?php if (isset($_GET['edit'])) { 
                $edit=$_GET['edit'];
                $eq="SELECT * FROM noticeboard WHERE id=$edit";
                $res=mysqli_query($con,$eq);
                $editrow=mysqli_fetch_array($res);

            ?>
            
              <form method="post" class="mb-5">
                <h3>Edit a post :</h3>
                <div class="row">
                  <div class="col-md-12">
                    <div class="form-group">
                        <label class="font-weight-bold" >Notice Title :</label>
                        <input type="text" class="form-control" name="title" value="<?= $editrow['title'] ?>" />
                    </div>
                  </div>

                  <div class="col-md-12">
                    <div class="form-group">
                      <label class="font-weight-bold">Notice Details :</label>
                      <textarea class="form-control" name="details"><?= $editrow['details'] ?></textarea>
                    </div>
                  </div>

                  <div class="col-md-12">
                    <div class="form-group">
                      <label class="font-weight-bold" >Notice Date :</label>
                      <input type="date" class="form-control"   name="date" required="1" value="<?= $editrow['dates'] ?>">
                    </div>
                  </div>
                  
                    
                    <div class="col-md-12">
                        <input type="hidden" name="updateid" value="<?= $editrow['id'] ?>">
                        <input type="submit" class="btn my-btn mt-4" value="Update" name="update"/>
                    </div>
                </div>
              </form>


            <?php } ?>

        <?php
        
        if (isset($_GET['entry'])) { 

            if (isset($_POST['submit'])) {

              $title = addslashes($_POST['title']); 
              $details = addslashes($_POST['details']);
              $date = $_POST['date'];

              $query="INSERT INTO noticeboard (title,details,dates) VALUES ('$title','$details','$date')";

            $q=mysqli_query($con,$query);
            if ($q) {
              $msg= '<div class="alert alert-success" role="alert">Data Added Successfully!</div>';

            }else{
              $msg= '<div class="alert alert-danger" role="alert">Error! Please try again later.</div>';
            }
              
            }

        ?>

          <form method="post" action="" enctype="multipart/form-data" >
          <div class="form-in">
            <div class="row">
              

              <div class="col-md-12">
                <div class="form-group">
                  <label class="font-weight-bold" >Notice Title :</label>
                  <input type="text" class="form-control"  placeholder="Notice Title" name="title" required="1">
                </div>
              </div>

              <div class="col-md-12">
                <div class="form-group">
                    <label class="font-weight-bold">Notice Details :</label>
                    <textarea class="form-control" placeholder="Notice Details" name="details" id="mywysiwyg" rows="5"></textarea>
                  </div>
              </div>

              <div class="col-md-12">
                <div class="form-group">
                  <label class="font-weight-bold" >Notice Date :</label>
                  <input type="date" class="form-control"   name="date" required="1">
                </div>
              </div>

              <div class="col-md-12">
                <input type="submit" class="btn my-btn" name="submit" value="Submit" />
              </div>
            </div>
          </div>
        </form>

      <?php } ?>
      <br>
      <?= $msg; ?>

          <br>
        <h4>All Notice</h4>
        <table class="table tbl_stripe">
            <tr>
              <th>Notice Title</th>
              <th>Notice Details</th>
              <th>Date</th>
              <th>Action</th>
            </tr>
            <?php
$limit = 10;  
if (isset($_GET["page"])) { $page  = $_GET["page"]; } else { $page=1; };  
$start_from = ($page-1) * $limit;  
  
$sql = "SELECT * FROM noticeboard ORDER BY id DESC LIMIT $start_from, $limit";  
$result = mysqli_query($con, $sql);
                while($row=mysqli_fetch_array($result))
                {
                  echo '<tr>';
                  echo '<td align="left">'.  nl2br(substr($row['title'],0,100)).'</td>';
                  echo '<td align="left">'.  nl2br(substr($row['details'],0,300)).'</td>';
                  echo '<td>'. $row['dates'].'</td>';
                  echo '<td>
                  <a href="?edit='. $row['id'].'" class="btn btn-warning" >Edit</a>
                  <a href="?deleteid='. $row['id'].'" class="btn btn-danger" onClick="return confirm(\' Are you sure to delete data..\') ">Delete</a>  
                  </td>';
                  echo '</tr>';
                }

            ?>
        </table>
<?php  
$sql = "SELECT COUNT(id) FROM noticeboard";  
$rs_result = mysqli_query($con, $sql);  
$row = mysqli_fetch_array($rs_result);  
$total_records = $row[0];  
$total_pages = ceil($total_records / $limit);  
$pagLink = '<nav aria-label="Page navigation example">
              <ul class="pagination">';  
for ($i=1; $i<=$total_pages; $i++) {  
             $pagLink .= "<li class='page-item'><a class='page-link' href='?page=".$i."'>".$i."</a></li>";  
};  
echo $pagLink . "</ul></nav>";  
?>
        
        </div>
      </div>
  </div>


<?php include('footer.php'); ?>